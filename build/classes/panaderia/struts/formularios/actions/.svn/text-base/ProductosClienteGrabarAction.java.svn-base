...
public class ProductosClienteGrabarAction extends Action {
	...
	{              		
		...
		try{
			...
			if(request.getParameter("delete")==null){
				productosCliente.cargaDatosFormularioStruts(request);
				resultado = productosCliente.inserta();
			}else{
				String[] chkValues = request.getParameterValues("checkList");	
				for(int i=0; i<chkValues.length; i++){
					productosCliente.setPclId(chkValues[i]);
					resultado = productosCliente.elimina();
				}
			}
