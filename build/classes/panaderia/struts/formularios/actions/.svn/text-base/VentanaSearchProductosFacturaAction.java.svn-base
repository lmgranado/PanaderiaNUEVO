...
public class VentanaSearchProductosFacturaAction extends Action {
	...
	{              		
		...
		try{
	        ...
	        if(request.getParameter("filtro")!=null && !request.getParameter("filtro").equals("")){
				request.getSession().setAttribute("FPRODUCTOS", productos); //El filtro será F+Nombre de la tabla
			}
