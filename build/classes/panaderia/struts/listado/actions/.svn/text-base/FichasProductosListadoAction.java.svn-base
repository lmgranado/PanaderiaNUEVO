...
public class FichasProductosListadoAction extends Action {
	...
	{              
		...
		FichasProductos fichas = (FichasProductos) form;
		bBotoneraListado botonera = new bBotoneraListado();
		
		try{
	        // Parámetros por defecto para la primera vez que se muestra la lista.
	        boolean empty = false;
	        String tableName = "LISTAFICHASPRODUCTOS"; // Este es el parámetro que se debe poner en la propiedad "name" del DisplayTag.
	        String sortDirection = IPaginatedListTest.ASC;
	        String sortCriterion = "FPRODNOMBREFICHA";//Este es el nombre de la propiedad de la columna por la que se pagina la primera vez
	        int pageSize = 100;
	        
	        if(request.getParameter("filtro")!=null && !request.getParameter("filtro").equals("")){
				request.getSession().setAttribute("FFICHAS_PRODUCTOS", fichas); //El filtro será F+Nombre de la tabla
			}
	        
	        PaginatedListTest lista = PaginatedListTest.getPaginatedList(request,
	                empty, tableName, sortDirection, sortCriterion, pageSize); 
	        Object[] consulta = FichasProductos.getFichasProductosLista( lista );
    		...
    		request.setAttribute("LISTAFICHASPRODUCTOS", lista);
