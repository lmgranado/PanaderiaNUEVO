...
public class GenerarListadoPreciosPDF extends PdfPageEventHelper{
    ...
    public GenerarListadoPreciosPDF(String tipo){
	...
	public void inicializaListado() throws DocumentException, FileNotFoundException{
        ...
        writer.setPageEvent(new GenerarListadoPreciosPDF(tipo));
	}
    ...
    public void generaListadoPrecios( HttpServletRequest request, HttpServletResponse response, ArrayList productos ) throws DocumentException, IOException{
	...
	public void abrirDocumento(){
	    ...
	    document.addCreator("Listado de Precios");
	...
	public void generaCuerpoListado( ArrayList productos, HttpServletRequest request ) throws DocumentException, IOException{
		...
		int contador = 1;
		int columnas = 4;
		int[] tamanios = new int[]{10,20,20,15};
		if(tipo.equals("cliente")){
			columnas = 6;
			tamanios = new int[]{10,20,20,15,20,15};
		}
		
		PdfPTable tablaCuerpo = new PdfPTable(columnas);
		try{
			tablaCuerpo.setWidthPercentage(100);
			tablaCuerpo.setWidths(tamanios);
	      ...
	      for(int i=0; i<productos.size(); i++){
	          ...
	          //FAMILIA
	          celda = new PdfPCell(new Phrase( producto[2], fuenteParrafo));
	          celda.setColspan(1);
	          ...
	          if(tipo.equals("cliente")){
	          //CLIENTE
		          celda = new PdfPCell(new Phrase( (producto[4]==null)?"":producto[4], fuenteParrafo));
		          celda.setColspan(1);
		          celda.setHorizontalAlignment(Element.ALIGN_CENTER);
		          celda.setBorderWidth(0); 
		          celda.setBorderWidthRight( 0.5f );
		          ...
		          //PRECIO CLIENTE
		          celda = new PdfPCell(new Phrase( (producto[5]==null || producto[5].equals(""))?"":Utils.formateaCantidad(producto[5]), fuenteParrafo));
		          celda.setColspan(1);
		          celda.setHorizontalAlignment(Element.ALIGN_CENTER);
		          celda.setBorderWidth(0); 
		          celda.setBorderWidthRight( 0.5f );
	...
	public PdfPCell generaCabeceraCuerpoListado( PdfPTable tablaCuerpo ){
		 ...
		 PdfPCell celda = new PdfPCell(new Phrase("", fuenteParrafo));
		  ...
		  //REFERENCIA DEL PRODUCTO
	      celda = new PdfPCell(new Phrase("REF.", fuenteParrafoNegrita));
	      celda.setColspan(1);
...
//	    DESCRIPCION DEL PRODUCTO
	      ...
	      if(tipo.equals("cliente")){
		      ...
		      celda = new PdfPCell(new Phrase("PRECIO CLIENTE", fuenteParrafoNegrita));
		      celda.setColspan(1);
		      celda.setPaddingBottom( 5 );
		      celda.setHorizontalAlignment(Element.ALIGN_CENTER);
		      celda.setBorder( 0 );
