...
public class UsuariosListadoAction extends Action {
	...
	{              
		...
		try{
	        ...
	        boolean empty = false;
	        String tableName = "LISTAUSUARIOS"; // Este es el parámetro que se debe poner en la propiedad "name" del DisplayTag.
	        ...
	        if(request.getParameter("filtro")!=null && !request.getParameter("filtro").equals("")){
				request.getSession().setAttribute("FUSUARIOS", usuarios); //El filtro será F+Nombre de la tabla
			}
