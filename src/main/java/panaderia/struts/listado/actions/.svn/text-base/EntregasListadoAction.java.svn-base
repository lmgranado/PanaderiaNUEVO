...
public class EntregasListadoAction extends Action {
	...
	{              
		...
		try{
	        ...
	        boolean empty = false;
	        String tableName = "LISTAENTREGAS"; // Este es el parámetro que se debe poner en la propiedad "name" del DisplayTag.
	        ...
	        if(request.getParameter("filtro")!=null && !request.getParameter("filtro").equals("")){
	        	...
	        	request.getSession().setAttribute("FENTREGAS", entrega); //El filtro será F+Nombre de la tabla
			}
