...
public class FormasPagoListadoAction extends Action {
	...
	{              
		...
		try{
	        ...
	        boolean empty = false;
	        String tableName = "LISTAFORMASPAGO"; // Este es el parámetro que se debe poner en la propiedad "name" del DisplayTag.
	        String sortDirection = IPaginatedListTest.ASC;
	        String sortCriterion = "FPDESCRIPCION";//Este es el nombre de la propiedad de la columna por la que se pagina la primera vez
	        int pageSize = 100;
	        	        
	        request.setAttribute("fpId", request.getParameter("fpId"));
	        formas.setFpId( request.getParameter("fpId") );
			request.getSession().setAttribute("FRUTAS", formas); //El filtro será F+Nombre de la tabla
	        
	        request.setAttribute("LISTAFORMASPAGO",FormasPago.getAllFormasPago());; 
	        
	        PaginatedListTest lista = PaginatedListTest.getPaginatedList(request,
	                empty, tableName, sortDirection, sortCriterion, pageSize); 
	        Object[] consulta = FormasPago.getFormasPagoLista(lista);
    		...
    		request.setAttribute("LISTAFORMASPAGO", lista);
