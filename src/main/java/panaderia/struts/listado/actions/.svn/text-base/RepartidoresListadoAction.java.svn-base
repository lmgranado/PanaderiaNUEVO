...
public class RepartidoresListadoAction extends Action {
	...
	{              
		...
		try{
	        ...
	        boolean empty = false;
	        String tableName = "LISTAREPARTIDORES"; // Este es el parámetro que se debe poner en la propiedad "name" del DisplayTag.
	        ...
	        if(request.getParameter("filtro")!=null && !request.getParameter("filtro").equals("")){
				request.getSession().setAttribute("FREPARTIDORES", repartidores); //El filtro será F+Nombre de la tabla
			}
