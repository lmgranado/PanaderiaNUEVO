...
public class ProveedoresListadoAction extends Action {
	...
	{              
		...
		try{
	        ...
	        boolean empty = false;
	        String tableName = "LISTAPROVEEDORES"; // Este es el parámetro que se debe poner en la propiedad "name" del DisplayTag.
	        ...
	        if(request.getParameter("filtro")!=null && !request.getParameter("filtro").equals("")){
				request.getSession().setAttribute("FPROVEEDORES", proveedor); //El filtro será F+Nombre de la tabla
			}
