...
public class FacturasVentaListadoAction extends Action {
	...
	{              
		...
		try{
	        ...
	        boolean empty = false;
	        String tableName = "LISTAFACTURASVENTA"; // Este es el parámetro que se debe poner en la propiedad "name" del DisplayTag.
	        ...
	        if(request.getParameter("filtro")!=null && !request.getParameter("filtro").equals("")){
				request.getSession().setAttribute("FFACTURAS_VENTA", facturas); //El filtro será F+Nombre de la tabla
			}
