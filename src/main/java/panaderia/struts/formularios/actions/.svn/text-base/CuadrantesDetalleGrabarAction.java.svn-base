...
public class CuadrantesDetalleGrabarAction extends Action {
	...
	{              		
		...
		try{
			...
			if(request.getParameter("delete")==null){
				cuadrantesDetalle.cargaDatosFormularioStruts(request);
				resultado = cuadrantesDetalle.inserta();
			}else{
				String[] chkValues = request.getParameterValues("checkList");	
				for(int i=0; i<chkValues.length; i++){
					cuadrantesDetalle.setCudId(chkValues[i]);
					resultado = cuadrantesDetalle.elimina();
				}
			}
