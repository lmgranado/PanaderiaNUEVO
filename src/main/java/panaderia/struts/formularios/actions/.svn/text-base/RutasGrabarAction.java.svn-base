...
public class RutasGrabarAction extends Action {
	...
	{              		
		...
		try{
			...
			if(request.getParameter("delete")==null){
				rutas.cargaDatosFormularioStruts(request);
				resultado = rutas.inserta();
			}else{
				String[] chkValues = request.getParameterValues("checkList");	
				for(int i=0; i<chkValues.length; i++){
					rutas.setRutId( chkValues[i] );
					resultado = rutas.elimina();
				}
			}
