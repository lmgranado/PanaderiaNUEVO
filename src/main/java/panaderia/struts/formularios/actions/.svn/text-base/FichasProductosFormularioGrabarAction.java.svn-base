...
public class FichasProductosFormularioGrabarAction extends Action {
	...
	{              		
		...
		try{
			...
			if(Utils.skipNull(fprodId).equals("")){				
				resultado = ficha.inserta();
				ficha = FichasProductos.getFichasProductosByFprodId(Integer.toString( resultado ));
			}else{
				resultado = ficha.actualiza();
				ficha = FichasProductos.getFichasProductosByFprodId( fprodId );
			}
			
			request.setAttribute( "Ficha" , ficha );
			request.setAttribute( "fprodId" , ficha.getFprodId() );
