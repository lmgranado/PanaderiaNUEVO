...
public class PeriodosFacturacionGrabarAction extends Action {
	...
	{              		
		...
		try{
			...
			if(request.getParameter("delete")==null){
				periodos.cargaDatosFormularioStruts(request);
				resultado = periodos.inserta();
			}else{
				String[] chkValues = request.getParameterValues("checkList");	
				for(int i=0; i<chkValues.length; i++){
					periodos.setPfId( chkValues[i] );
					resultado = periodos.elimina();
				}
			}
