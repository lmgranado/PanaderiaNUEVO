...
public class FamiliasGrabarAction extends Action {
	...
	{              		
		...
		try{
			...
			familias.cargaDatosFormularioStruts(request);
			
			if( Utils.empty(familias.getFamId()) ){
				resultado = familias.inserta();
			}
			else{
				resultado = familias.actualiza();
			}
