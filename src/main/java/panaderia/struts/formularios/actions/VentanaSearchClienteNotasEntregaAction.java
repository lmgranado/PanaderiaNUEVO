...
public class VentanaSearchClienteNotasEntregaAction extends Action {
	...
	{              		
		...
		try{
	        ...
	        if(request.getParameter("filtro")!=null && !request.getParameter("filtro").equals("")){
				request.getSession().setAttribute("FCLIENTESVN", cliente); //El filtro será F+Nombre de la tabla
			}
