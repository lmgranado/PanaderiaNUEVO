...
public class VentanaSearchClienteFacturaAction extends Action {
	...
	{              		
		...
		//Pasamos de que tipo de Factura se trata
		request.setAttribute( "fvTipo", Utils.skipNull(request.getParameter("fvTipo")) );	
		...
		try{
	        ...
	        request.getSession().setAttribute("FCLIENTESVF", cliente);
	        
	        if(request.getParameter("filtro")!=null && !request.getParameter("filtro").equals("")){
				request.getSession().setAttribute("FCLIENTESVF", cliente); //El filtro será F+Nombre de la tabla
			}
