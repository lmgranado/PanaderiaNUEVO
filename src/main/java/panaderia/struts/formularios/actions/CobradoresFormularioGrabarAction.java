...
public class CobradoresFormularioGrabarAction extends Action {
	...
	{              		
		...
		int resultado = -2;
		try{
			String cobId = request.getParameter("cobId");
			Cobradores cobrador = Cobradores.getCobradoresByCobId( cobId );
			cobrador.cargaDatosFormularioStruts(request);
			if(Utils.skipNull( cobId ).equals(""))
				resultado = cobrador.inserta();
			else
				resultado = cobrador.actualiza();
