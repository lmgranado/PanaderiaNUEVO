	...
	String sql = "SELECT * FROM componentes_producto cp " +
  	...
  	try{
    	...
    	if(!Utils.empty(componentesproductoFiltro.getCpCantidad()))
    		sqlAñadido += "AND " + CPCANTIDAD + " = '" + componentesproductoFiltro.getCpCantidad() + "'";
    	if(!Utils.empty(componentesproductoFiltro.getCpId()))
    		sqlAñadido += "AND " + CPID + " = '" + componentesproductoFiltro.getCpId() + "'";
    	if(!Utils.empty(componentesproductoFiltro.getCpProdIdFabricado()))
    		sqlAñadido += "AND " + CPPRODIDFABRICADO + " = '" + componentesproductoFiltro.getCpProdIdFabricado() + "'";
    	if(!Utils.empty(componentesproductoFiltro.getCpProdIdUsado()))
    		sqlAñadido += "AND " + CPPRODIDUSADO + " = '" + componentesproductoFiltro.getCpProdIdUsado() + "'";
  	}catch(Exception e){
  		System.out.println(e);
  	}
    return UtilesDAO.generaSqlListadoPaginado(sql + sqlAñadido, listaPaginada, componentesproducto);
  }
}
