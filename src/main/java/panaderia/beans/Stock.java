	...
	String sql = "SELECT st.*, fc.*, fcd.* FROM stock st " +
  	...
  	try{
    	...
    	if(!Utils.empty(stockFiltro.getStCantidadFinal()))
    		sqlAñadido += "AND " + STCANTIDADFINAL + " = '" + stockFiltro.getStCantidadFinal() + "'";
    	if(!Utils.empty(stockFiltro.getStCantidadInicial()))
    		sqlAñadido += "AND " + STCANTIDADINICIAL + " = '" + stockFiltro.getStCantidadInicial() + "'";
    	if(stockFiltro.getStFCaducidad()!=null)
    		sqlAñadido += "AND " + STFCADUCIDAD + " = '" + stockFiltro.getStFCaducidad() + "'";
    	if(!Utils.empty(stockFiltro.getStId()))
    		sqlAñadido += "AND " + STID + " = '" + stockFiltro.getStId() + "'";
    	if(!Utils.empty(stockFiltro.getStProdId()))
    		sqlAñadido += "AND " + STPRODID + " = '" + stockFiltro.getStProdId() + "'";
    	if(!Utils.empty(stockFiltro.getStRegularizacion()))
    		sqlAñadido += "AND " + STREGULARIZACION + " = '" + stockFiltro.getStRegularizacion() + "'";
    	if(!Utils.empty(stockFiltro.getStSalidas()))
    		sqlAñadido += "AND " + STSALIDAS + " = '" + stockFiltro.getStSalidas() + "'";
    	
    	if( stockFiltro.getStStockCero().equals("off") ){
    		sqlAñadido += "AND " + STCANTIDADFINAL + " > 0 ";
    	}
    	
    	
//    	Introducimos busqueda por rango de fechas
    	if( stockFiltro.getStFechaDesde()!=null && stockFiltro.getStFechaHasta()!=null )
    		sqlAñadido += "AND fc_fecha BETWEEN '"+stockFiltro.getStFechaDesde()+"' AND '"+stockFiltro.getStFechaHasta()+"' ";
    	else if( stockFiltro.getStFechaDesde()!=null ) 
    		sqlAñadido += "AND fc_fecha >= '" + stockFiltro.getStFechaDesde() + "'";
    	else if( stockFiltro.getStFechaHasta()!=null )
    		sqlAñadido += "AND fc_fecha <= '" + stockFiltro.getStFechaHasta() + "'";
  	}catch(Exception e){
  		System.out.println(e);
  	}
    return UtilesDAO.generaSqlListadoPaginado(sql + sqlAñadido, listaPaginada, stock);
  }
}
