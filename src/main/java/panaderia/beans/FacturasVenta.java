	...
	String sql = "SELECT sum(fv.fv_importe_pendiente)"+
  	...
  	try{
    	...
    	if(!Utils.empty(facturasventaFiltro.getFvClId()))
    		sqlAñadido += "AND " + FVCLID + " = '" + facturasventaFiltro.getFvClId() + "'";
    	if(facturasventaFiltro.getFvFecha()!=null)
    		sqlAñadido += "AND " + FVFECHA + " = '" + facturasventaFiltro.getFvFecha() + "'";
    	if(!Utils.empty(facturasventaFiltro.getFvId()))
    		sqlAñadido += "AND " + FVID + " like '%" + facturasventaFiltro.getFvId() + "%'";
    	if(!Utils.empty(facturasventaFiltro.getFvIva()))
    		sqlAñadido += "AND " + FVIVA + " = '" + facturasventaFiltro.getFvIva() + "'";
    	if(!Utils.empty(facturasventaFiltro.getFvNumeroFactura()))
    		sqlAñadido += "AND " + FVNUMEROFACTURA + " like '%" + facturasventaFiltro.getFvNumeroFactura() + "%'";
    	if(!Utils.empty(facturasventaFiltro.getFvPagada()))
    		sqlAñadido += "AND " + FVPAGADA + " = '" + facturasventaFiltro.getFvPagada() + "'";
    	if(!Utils.empty(facturasventaFiltro.getFvTotal()))
    		sqlAñadido += "AND " + FVTOTAL + " = '" + facturasventaFiltro.getFvTotal() + "'";
    	if(!Utils.empty(facturasventaFiltro.getFvImportePendiente()))
    		sqlAñadido += "AND " + FVIMPORTEPENDIENTE + " = '" + facturasventaFiltro.getFvImportePendiente() + "'";
    	if(!Utils.empty(facturasventaFiltro.getFvObservaciones()))
    		sqlAñadido += "AND UPPER(" + FVOBSERVACIONES + ") like UPPER('%" + facturasventaFiltro.getFvObservaciones() + "%')";
    	if( facturasventaFiltro.getClDatosRelacionados()!= null && !Utils.empty( ( facturasventaFiltro.getClDatosRelacionados()[0] ) ) )
    		sqlAñadido += "AND UPPER(" + CLNOMBRE + ") LIKE UPPER('%" + ( facturasventaFiltro.getClDatosRelacionados()[0])+ "%')";
    	if( !Utils.empty( facturasventaFiltro.getClPfId() ) )
    		sqlAñadido += "AND " + CLPFID + " = '" + ( facturasventaFiltro.getClPfId())+ "'";
    	if( !Utils.empty( facturasventaFiltro.getFvCierre() ) )
    		sqlAñadido += "AND " + FVCIERRE + " = '" + ( facturasventaFiltro.getFvCierre())+ "'";
    	//Nuevo
    	if( !Utils.empty( facturasventaFiltro.getClCobId() ) )
    		sqlAñadido += "AND " + CLCOBID + " = '" + ( facturasventaFiltro.getClCobId())+ "'";
    	...
    	if( facturasventaFiltro.getFvFechaDesde()!=null && facturasventaFiltro.getFvFechaHasta()!=null )
    		sqlAñadido += "AND " +FVFECHA+" BETWEEN '"+facturasventaFiltro.getFvFechaDesde()+"' AND '"+facturasventaFiltro.getFvFechaHasta()+"' ";
    	else if( facturasventaFiltro.getFvFechaDesde()!=null ) 
    		sqlAñadido += "AND " + FVFECHA + " >= '" + facturasventaFiltro.getFvFechaDesde() + "'";
    	else if( facturasventaFiltro.getFvFechaHasta()!=null )
    		sqlAñadido += "AND " + FVFECHA + " <= '" + facturasventaFiltro.getFvFechaHasta() + "'";
    	
    	if( facturasventaFiltro.getFvFechaPago()!=null )
    		sqlAñadido += "AND " + FVFECHAPAGO + " = '" + facturasventaFiltro.getFvFechaPago() + "'";
    	
    	
  	}catch(Exception e){
  		System.out.println(e);
  	}
    return EntidadBean.consultarValor(sql + sqlAñadido);
  }
	...
	String sql = "SELECT fv.*,"+ 
  	...
  	try{
    	...
    	if( !Utils.empty(facturasventaFiltro.getFvTipo()) && !facturasventaFiltro.getFvTipo().equals("3"))
    		sqlAñadido += "AND " + FVTIPO + " = '" + facturasventaFiltro.getFvTipo() + "'";
    	
    	if( !Utils.empty(facturasventaFiltro.getFvTipo()) && facturasventaFiltro.getFvTipo().equals("3"))
    		sqlAñadido += "AND " + FVFVREF + " IS NOT NULL ";
    	
  	}catch(Exception e){
  		System.out.println(e);
  	}
    return UtilesDAO.generaSqlListadoPaginado(sql + sqlAñadido, listaPaginada, facturasventa);
  }
