	...
	function doSubmit(){
		...
		if(validateFields(vForm.name)){
			...
			if(vForm.clProporcionIva.value.replace(",",".") > 100){
				alert('La proporción de IVA no puede ser superior a 100%');
				return false;
			}
			if( vForm.clProporcionIva.value.replace(",",".") > 0  && document.getElementById('clIdEmpresa').value == ""){
				alert('Debe elegir un tipo de empresa para la facturación.');
				return false;
			}
			else{
				if( document.getElementById('existecif').value > 0 ){
					var x = confirm( 'Atención, existe un cliente con el mismo CIF.\n ¿Desea continuar?' );
					if( x ){
          ...
          <td align="center" colspan='4' style="padding-left: 20px;"> 
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="texto">
              <tr> 
				...
				<td style='width: 4%'>&nbsp;</td>
                <td style='width: 11%' align="left" class="texto_negrita">Proporción IVA</td>
