	...
	function doImprimirEtiquetas(){
		if(hayChecked('frmListado')){
			if(confirm('Va a imprimir las etiquetas seleccionadas, ¿Desea continuar?')){
				document.getElementById('frmDetalle').action='doGenerarEtiquetasCompras.do';
...
<html:form action="/doVentanaImprimirEtiquetasCompras" styleId="frmDetalle"  method="post">	
					...
					<c:forEach items="${LISTADETALLES}" var="detalle">
						...
						<tr class="${estilo}">							
							<td class="columna" style="text-align: left;">
								<c:out value="${detalle.fcdProducto}"/>
							</td>
							...
							<td class="columna" style="text-align: right;">
								<fmt:formatNumber type="number"  pattern="#" var="cantidad">${detalle.fcdCantidad}</fmt:formatNumber>
								<c:choose>
									<c:when test="${cantidad < '0'}">
										<input onblur="this.value=formato_entero(this.value)" style="text-align: right;font-size: 11px;" id="cantidad" name="fcdId_${detalle.fcdId}"  value="0">
									</c:when>
									<c:otherwise>
										<input onblur="this.value=formato_entero(this.value)" style="text-align: right;font-size: 11px;" id="cantidad" name="fcdId_${detalle.fcdId}" value="${cantidad}">
									</c:otherwise>
								</c:choose>
							</td>
						</tr>
