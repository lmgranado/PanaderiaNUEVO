...
<script type="text/javascript">
	...
	function doSubmit(){
		...
		if( validaCampos( idProducto, cantidad) ){
			document.getElementById('frmDetalle').action = 'doAlbaranesVentaDetalleGrabar.do?prodId=' + idProducto + '&cantidad='+ cantidad+'&avClId=${avClId}';
	...
	function validaCampos( idProducto, cantidad ){
		var correcto = true;
		if( idProducto == '-1' ){
			alert('Debe elegir algun producto para añadir al Albarán')
			correcto = false;
		}
		else if( cantidad < 1 ){
...
<html:form action="/doVentanaSearchProductosClienteAlbaran" styleId="frmDetalle"  method="post">
<table class="displaytag_tabla" style="margin: 0px 20px; width: 100%;" cellpadding="2" cellspacing="2">
	<html:hidden property="avClId" value="${avClId}"/>
	<tr>
	...
	<tr>
		...
		<td width="10%" style="font-family: Verdana; font-size: 11px; color: #666666">
			<input id="cantidad" onblur="javascript:this.value=formato_entero(this.value)" size="16" name="cantidad" type="text" value="${cantidad}">
		</td>
	</tr>
	<tr>
		<td width="6%" style="font-family: Verdana; font-size: 11px; color: #666666">
			<b>Introduzca el viaje: </b>
		</td>
		<td width="10%" style="font-family: Verdana; font-size: 11px; color: #666666">
			<input id="avdViaje" onblur="javascript:this.value=formato_entero(this.value)" size="16" maxlength="1" name="avdViaje" type="text">
		</td>
	</tr>
	<tr>
		<td colspan="4" style="">
	...
	<input type='hidden' value="${avId}" id="avId" name="avId" />
	<input type='hidden' value="${avClId}" name="avClId" id="avClId" />
	<input type='hidden' value="true" name="accion" />
</html:form>
<!-- Validacion -->

<script type="text/javascript">
	window.opener.location = 'doAlbaranesVentaFormulario.do?avId=${avId}&keyMapaActual=' + window.opener.document.getElementById('keyMapa').value;
</script>